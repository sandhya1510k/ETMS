...
import {
  ...
  FaLaptopCode, FaProjectDiagram, FaEnvelope, FaIdBadge
} from "react-icons/fa";
...
export default function TraineeDashboard({ isSidebarOpen }) {
  ...
  useEffect(() => {
    ...
    const id = localStorage.getItem("traineeId") || "CAML16515";
    const email = localStorage.getItem("traineeEmail") || "trainee@camelq.com";
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative">
            ...
            >
              ...
              {showProfileMenu ? (
                <FaChevronUp className="hidden md:inline text-gray-500 text-sm" />
              ) : (
                <FaChevronDown className="hidden md:inline text-gray-500 text-sm" />
              )}
            </button>
            {showProfileMenu && (
              <motion.div
              ...
              >
                <div className="px-4 py-2 text-sm text-gray-700 border-b border-gray-100">
                  <div className="flex items-center mb-2">
                    <FaIdBadge className="text-indigo-600 mr-2" />
                    <span className="font-medium">ID: {traineeId}</span>
                  </div>
                  <div className="flex items-center">
                    <FaEnvelope className="text-teal-600 mr-2" />
                    <span className="truncate">{traineeEmail}</span>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          {metrics.map((metric, i) => (
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <div className={`p-3 rounded-lg bg-opacity-20 ${metric.trend === "up" ? "bg-gray-100 text-teal-600" : metric.trend === "down" ? "bg-gray-100 text-red-600" : "bg-gray-100 text-gray-600"}`}>
                  {metric.icon}
                </div>
              </div>
          ...
          >
            ...
            <div className="space-y-4">
              {tasks.slice(0, 3).map((task, i) => (
                <div 
                  key={i} 
                  className="p-3 border border-gray-100 rounded-lg hover:bg-gray-50 transition cursor-pointer"
                  onClick={() => navigate("/tasks")}
                >
                  <div className="flex justify-between items-start">
